...
# Add New Routes and Functionalities Below

# Swimmer: Enroll in Lessons with Payment
@app.route('/swimmer_enroll/<int:session_id>', methods=['POST'])
def swimmer_enroll(session_id):
    if 'loggedin' in session and session.get('role') in ['Swimmer', 'Member']:
        swimmer_id = session['user_id']
        cursor = mysql.connection.cursor(MySQLdb.cursors.DictCursor)

        # Check for existing enrollment
        cursor.execute('SELECT * FROM booking WHERE swimmer_id = %s AND session_id = %s',
                       (swimmer_id, session_id))
        if cursor.fetchone():
            flash('You are already enrolled in this session.', 'warning')
            return redirect(url_for('swimmer_homepage'))
        ...
        # Enroll swimmer
        cursor.execute('''
            INSERT INTO booking (swimmer_id, session_id, is_completed, payment_method, is_payment_completed)
            VALUES (%s, %s, FALSE, NULL, FALSE)
        ''', (swimmer_id, session_id))
        mysql.connection.commit()
        flash('Enrolled successfully. Please proceed with payment.', 'success')
        return redirect(url_for('swimmer_lesson_payment', session_id=session_id))
    else:
        flash('Unauthorized access!', 'danger')
        return redirect(url_for('login'))

# Swimmer: Make Payment for Enrolled Lesson
@app.route('/swimmer_lesson_payment/<int:session_id>', methods=['GET', 'POST'])
def swimmer_lesson_payment(session_id):
    if 'loggedin' in session and session.get('role') in ['Swimmer', 'Member']:
        if request.method == 'POST':
            payment_method = request.form['payment_method']
            swimmer_id = session['user_id']

            # Update payment method and completion status
            cursor = mysql.connection.cursor(MySQLdb.cursors.DictCursor)
            cursor.execute('''
                UPDATE booking
                SET payment_method = %s, is_payment_completed = %s
                WHERE swimmer_id = %s AND session_id = %s
            ''', (payment_method, payment_method == 'Cash', swimmer_id, session_id))
            mysql.connection.commit()

            flash('Payment completed successfully.', 'success')
            return redirect(url_for('swimmer_homepage'))

        return render_template('swimmer_lesson_payment.html', session_id=session_id)
    else:
        flash('Unauthorized access!', 'danger')
        return redirect(url_for('login'))

# Coach: View and Edit Lessons
@app.route('/coach_lessons', methods=['GET'])
def coach_lessons():
    if 'loggedin' in session and session.get('role') == 'Coach':
        coach_id = session['user_id']
        cursor = mysql.connection.cursor(MySQLdb.cursors.DictCursor)
        cursor.execute('SELECT * FROM lesson WHERE coach_id = %s', (coach_id,))
        lessons = cursor.fetchall()
        return render_template('coach_lessons.html', lessons=lessons)
    else:
        flash('Unauthorized access!', 'danger')
        return redirect(url_for('login'))
...
def edit_lesson(lesson_id):
    if 'loggedin' in session and session.get('role') == 'Coach':
        ...
        if request.method == 'POST':
            ...
            cursor.execute('''
                UPDATE lesson
                SET description = %s, date = %s, start_time = %s, end_time = %s, capacity = %s
                WHERE session_id = %s
            ''', (description, date, start_time, end_time, capacity, lesson_id))
            mysql.connection.commit()
            flash('Lesson updated successfully.', 'success')
            return redirect(url_for('coach_lessons'))

        cursor.execute('SELECT * FROM lesson WHERE session_id = %s', (lesson_id,))
        lesson = cursor.fetchone()
